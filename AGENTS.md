...
FIRERED at the same quality bar it was built with. `CLAUDE.md` points here.

## What this project is

A fan-made, FireRed-style monster-catching RPG in **vanilla HTML/CSS/JS**.
...
5. **Git:** this repo intentionally has a SINGLE root commit. Changes are
   amended into it (`git add -A && git commit --amend --no-edit`). NEVER
   push — the user pushes (history rewrites need their force-push).
...
README + the "single-shot prompt" section updated whenever behavior changes.
...
git add -A && git commit --amend --no-edit   # fold work into the root commit
```
